...
from BMsgd import BM_SGD, BM_VGD
...
maxdegree = 4
for degree in range(4, maxdegree + 2,2):
	print('\n')
	print('degree:', degree, 'of', maxdegree,'\n')
	...
	for numVar in range(3, maxnumVar + 1):
		print('\n')
		print('numVar', numVar, 'of', maxnumVar,'\n')
		...
		for i in range(outerloop):
			print('outerloop:',i+1,'of',outerloop)
			Q = make_random_Q(n)
			...
			for j in range(innerloops):
				print('Innerloop:',j+1,'of',innerloops)

			# R,isInfeasible,R_init = augmented_lagrangian(A_mats, p, n, m, r, printing = False)
				R,cost,R_init = BM_VGD(A_mats, p, n, r, lb = -1, ub = 1, step = 1e-4, maxsteps = 0, precision = 1e-10)
				...
				if isInfeasible:
					data.append([Q,p,R_init,cost,numVar,degree,r,outerloop])
					print('Congrats! Infeasibility detected.')

				savedata = True
				if savedata:
					iteration = 1
					if data:
						filename = 'VGDdata'+'numVar'+str(numVar)+'degree'+str(degree) + 'Iter' + str(iteration)
